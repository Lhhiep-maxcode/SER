    ...
    for token_num in range(1,max_length):
        ...
        while idx_batch < bsz:
            ...
            if end_sig[idx_batch]==1:
                ...
                new_target_key_cache = []
                new_target_value_cache = []
                for key, value in zip(target_key_cache, target_value_cache):
                    new_target_key_cache.append(torch.concat(
                        [key[:delete_idx], key[delete_idx+1:]], dim=0))
                    new_target_value_cache.append(torch.concat(
        ...
        if prefix_length==len(chosen_index[0]):
            ...
            target_past_key_values = replace_cache_from_key_value_lists(
                target_past_key_values,
                [new_key[idx_L] for idx_L in range(L)],
                [new_value[idx_L] for idx_L in range(L)],
            )
