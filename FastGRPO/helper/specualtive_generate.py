...
def speculative_generate(model, input_ids, attention_mask, tokenizer,
    ...
    def model_forward(model,input_ids,attention_mask,past_key_values,position_ids=None):
        ...
        def native_target_forward():
            """Use the target model's own Qwen3 forward for RoPE/cache handling.

            FastGRPO's original helper manually iterates decoder layers and
            passes `position_embeddings` into each layer.  Newer Qwen3
            Transformers builds can use a rotary sub-dimension that does not
            match the exported cos/sin shape, causing q[..., 32] vs cos[..., 128]
            or q[..., 32] vs cos[..., 16] errors.  The native model forward
            builds the correct rotary embeddings internally while still
            accepting FastGRPO's custom 4D tree attention mask and DynamicCache.
            """
        ...
        def rotary_dim_candidates(position_embeddings, decoder_layer):
            cos, _ = position_embeddings
            max_dim = int(cos.shape[-1])
            self_attn = getattr(decoder_layer, "self_attn", None)
            candidates = []
            for attr in ("rotary_dim", "head_dim", "qk_head_dim"):
                value = getattr(self_attn, attr, None)
                if value is not None:
                    candidates.append(int(value))
            q_proj = getattr(self_attn, "q_proj", None)
            num_heads = getattr(self_attn, "num_heads", None) or getattr(self_attn, "num_attention_heads", None)
            if q_proj is not None and num_heads:
                candidates.append(int(q_proj.out_features) // int(num_heads))
            # Some Qwen3 checkpoints expose cos/sin at the full head dimension,
            # while the layer applies RoPE only to a smaller rotary sub-dim.  The
            # server error q[..., 32] vs cos[..., 128] is exactly this case, so
            # include common smaller rotary dimensions and retry safely below.
            candidates.extend([max_dim, 128, 96, 80, 64, 48, 40, 32, 16])
            unique = []
            for value in candidates:
                if 0 < int(value) <= max_dim and int(value) not in unique:
                    unique.append(int(value))
            return unique or [max_dim]

        def align_position_embeddings(position_embeddings, q_len, rotary_dim):
        ...
        for decoder_layer in model.model.layers[: model.model.config.num_hidden_layers]:
            # Newer Qwen3/Transformers versions can expose rotary embeddings
            # whose last dimension is larger than the layer's attention head
            # dimension.  Slice cos/sin to the actual layer shape before tree
            # verification to avoid q[..., 32] vs cos[..., 128] mismatches.
            layer_outputs = None
            last_shape_error = None
            for rotary_dim in rotary_dim_candidates(position_embeddings, decoder_layer):
                layer_position_embeddings = align_position_embeddings(
                    position_embeddings,
    ...
    def cache_to_key_value_lists(cache):
        """Return mutable key/value lists for both old and new DynamicCache APIs."""
        ...
        if hasattr(cache, "to_legacy_cache"):
            ...
            return [item[0] for item in legacy_cache], [item[1] for item in legacy_cache]
        ...
        raise AttributeError("Unsupported cache object: cannot extract key/value tensors.")


    def replace_cache_from_key_value_lists(cache, key_cache, value_cache):
        """Rebuild/update a cache after FastGRPO manually edits KV tensors."""

        legacy_cache = tuple((key, value) for key, value in zip(key_cache, value_cache))
        if hasattr(cache, "key_cache") and hasattr(cache, "value_cache"):
            cache.key_cache = list(key_cache)
            cache.value_cache = list(value_cache)
            return cache
        if hasattr(cache.__class__, "from_legacy_cache"):
            try:
                return cache.__class__.from_legacy_cache(legacy_cache)
            except Exception:
                pass
        try:
            ...
            for layer, key, value in zip(cache.layers, key_cache, value_cache):
                ...
                for name in ("values", "value_cache", "value_states"):
                    if hasattr(layer, name):
                        ...
                        break
            return cache
        raise AttributeError("Unsupported cache object: cannot write key/value tensors.")
    ...
    with torch.amp.autocast(str(model.target_model.device),
        ...
        target_past_key_values = target_outputs.get('past_key_values', target_past_key_values)
    ...
    for token_num in range(1,max_length):
        ...
        while idx_batch < bsz:
            ...
            if end_sig[idx_batch]==1:
                ...
                target_key_cache, target_value_cache = cache_to_key_value_lists(target_past_key_values)
                for kv_idx in range(len(target_key_cache)):
                    key = target_key_cache[kv_idx]
                    value = target_value_cache[kv_idx]
                    
                    target_key_cache[kv_idx]=torch.concat(
                        [key[:delete_idx], key[delete_idx+1:]], dim=0)
                    target_value_cache[kv_idx]=torch.concat(
                        [value[:delete_idx], value[delete_idx+1:]], dim=0)
                target_past_key_values = replace_cache_from_key_value_lists(
        ...
        if prefix_length==len(chosen_index[0]):
            ...
            target_key_cache, target_value_cache = cache_to_key_value_lists(target_past_key_values)
            target_past_key_tensor=torch.stack(target_key_cache, dim=0)
            target_past_value_tensor=torch.stack(target_value_cache, dim=0)
            ...
            for idx_L in range(L):
                target_key_cache[idx_L] = new_key[idx_L]
                target_value_cache[idx_L] = new_value[idx_L]
            target_past_key_values = replace_cache_from_key_value_lists(
                target_past_key_values,
                target_key_cache,
                target_value_cache,
